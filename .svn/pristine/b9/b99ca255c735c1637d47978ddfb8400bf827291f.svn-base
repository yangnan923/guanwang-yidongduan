	...
	<view class="mosowe-swiper" :style="'height:'+height+'px'" style="margin-bottom: 20rpx;">
		...
		<!-- #ifndef MP-WEIXIN -->
				<swiper-item
					...
					<img :src="imageKey ? item[imageKey] : item" class="image" style="border-radius: 20px;box-shadow: rgba(151, 217, 55, 0.24) 0px 8px 25px;"  alt="">
					<!-- <image :src="imageKey ? item[imageKey] : item" class="image" v-if="swiperType === 'image'"></image> -->
					<!-- <view style="position: relative;top: -100rpx;"><img style="width: 99%" src="../../static/company/zhezhao1@3x.png" alt=""></view> -->
					<!-- 仅文字展示 -->
					<view v-if="item.des == ''">
						<text style="position: relative;top: -40px;z-index: 99999;
						font-size: 15px;
						font-family: FZLanTingYuanS-B-GB;
						font-weight: bold;
						color: #000000;text-align: center;" class="text">{{item.text}}
						</text>

					</view>
					<view v-else>
						<text style="position: relative;top: -54px;z-index: 99999;font-size: 15px;
						font-family: FZLanTingYuanS-B-GB;
						font-weight: bold;
						color: #000000;left: 20px;" class="text">{{item.text}}</text>
						<text style="position: relative;top: -52px;z-index: 99999;
						font-size: 13px;
						font-family: Microsoft YaHei;
						font-weight: 400;
						color: #7C7C7C;left: 20px;" class="text">{{item.des}}</text>
					</view>
					<!-- 图文左右轮播 -->
...
export default {
  ...
  methods: {
		...
		// banner点击时
		bannerClick (item) {
			//console.log(JSON.stringify(item.item.url));
			if(item.item.url){
